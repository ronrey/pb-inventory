...
export const SelectCoffeePercents: React.FC<Props> = ({ items, onChange }) => {
    const [edit, setEdit] = useState<BlendCoffee>({ coffee_id: '', percentage: 50 });
    ...
    const renderItem = (item: BlendCoffee, i: number) => {
        ...
        return (
            <div css={styles.coffeItemsContainer} key={i}>
                <SelectCoffee coffeeId={item.coffee_id} onChange={(cid) => handleCoffeeChange(cid, i)} />
                <TextField value={percentage}
                    type='number'
                    inputProps={{
                        min: 1,
                        step: 1,
                    ...
                    onChange={(event) => { handlePercentageChange(parseInt(event.target.value), i) }} />
                <Typography style={{ margin: 4 }} variant="subtitle2">%</Typography>
            </div>
        )
    }
    const renderEditItem = () => {
        ...
        return (
            <div css={styles.coffeItemsContainer}>
                <Button><AddIcon onClick={handleAdd} fontSize="small" /></Button>
                <SelectCoffee coffeeId={edit.coffee_id} onChange={handleEditCoffeeChange} />
                ...
                <TextField value={percentage}
                    ...
                    css={styles.percentageField}
                    onChange={(event) => { handleEditPercentageChange(parseInt(event.target.value)) }} />
                <Typography style={{ margin: 4 }} variant="subtitle2">%</Typography>
            </div>
        )
    }
    const handleAdd = () => {
        ...
        setEdit({ coffee_id: '', percentage: 50 });
        onChange(newItems);
    };
    return (<div>
